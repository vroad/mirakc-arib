...
TEST(ServiceFilterTest, HandlePacketDropsUnknownPidBeforeAnyFilter) {
  MockSource src;
  auto mock_handler = std::make_unique<MockServiceFilterTableHandler>();
  auto* handler_ptr = mock_handler.get();
  auto filter = std::make_unique<ServiceFilter>(kOption, std::move(mock_handler));
...
TEST(ServiceFilterTest, DemuxesPatAndCallsHandle) {
  TableSource src;
  auto mock_handler = std::make_unique<MockServiceFilterTableHandler>();
  auto* handler_ptr = mock_handler.get();
  auto filter = std::make_unique<ServiceFilter>(kOption, std::move(mock_handler));
...
TEST(ServiceFilterTest, DemuxesCat) {
  TableSource src;
  auto mock_handler = std::make_unique<MockServiceFilterTableHandler>();
  auto* handler_ptr = mock_handler.get();
  auto filter = std::make_unique<ServiceFilter>(kOption, std::move(mock_handler));
...
TEST(ServiceFilterTest, TotNotDemuxedWithoutTimeLimit) {
  TableSource src;
  auto mock_handler = std::make_unique<MockServiceFilterTableHandler>();
  auto* handler_ptr = mock_handler.get();
  auto filter = std::make_unique<ServiceFilter>(kOption, std::move(mock_handler));
...
TEST(ServiceFilterTest, TotDemuxedWithTimeLimit) {
  auto option = kOption;
...
TEST(ServiceFilterTest, ForwardsPatAfterPsiFilterIncludesPatPid) {
  TableSource src;
  auto mock_handler = std::make_unique<MockServiceFilterTableHandler>();
  auto* handler_ptr = mock_handler.get();
  auto filter = std::make_unique<ServiceFilter>(kOption, std::move(mock_handler));
...
TEST(ServiceFilterTest, ForwardsContentPidsListedInContentFilter) {
  TableSource src;
  auto mock_handler = std::make_unique<MockServiceFilterTableHandler>();
  auto* handler_ptr = mock_handler.get();
  auto filter = std::make_unique<ServiceFilter>(kOption, std::move(mock_handler));
...
TEST(ServiceFilterTest, ForwardsEmmPidsListedInEmmFilter) {
  TableSource src;
  auto mock_handler = std::make_unique<MockServiceFilterTableHandler>();
  auto* handler_ptr = mock_handler.get();
  auto filter = std::make_unique<ServiceFilter>(kOption, std::move(mock_handler));
...
TEST(ServiceFilterTest, SetDoneStopsForwardingAndReturnsFalse) {
  TableSource src;
  auto mock_handler = std::make_unique<MockServiceFilterTableHandler>();
  auto* handler_ptr = mock_handler.get();
  auto filter = std::make_unique<ServiceFilter>(kOption, std::move(mock_handler));
...
TEST(ServiceFilterTest, SetErrorPropagatesToExitCode) {
  TableSource src;
  auto mock_handler = std::make_unique<MockServiceFilterTableHandler>();
  auto* handler_ptr = mock_handler.get();
  auto filter = std::make_unique<ServiceFilter>(kOption, std::move(mock_handler));
...
TEST(ServiceFilterTest, SetPmtPidAddsPidToDemux) {
  TableSource src;
  auto mock_handler = std::make_unique<MockServiceFilterTableHandler>();
  auto* handler_ptr = mock_handler.get();
  auto filter = std::make_unique<ServiceFilter>(kOption, std::move(mock_handler));
