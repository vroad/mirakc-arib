...
constexpr const char kTestLogTag[] = "test";
...
TEST(TableValidatorTest, PatWithWrongPid) {
  ...
  EXPECT_FALSE(ValidatePat(kTestLogTag, pat, table));
}
...
TEST(TableValidatorTest, ValidTot) {
  ...
  EXPECT_TRUE(ValidateTot(kTestLogTag, tot));
}
...
TEST(TableValidatorTest, ValidEit) {
  ...
  EXPECT_TRUE(ValidateEit(kTestLogTag, eit));
}
...
TEST(TableValidatorTest, ValidNit) {
  ...
  EXPECT_TRUE(ValidateNit(kTestLogTag, nit));
}
...
TEST(TableValidatorTest, ValidSdt) {
  ...
  EXPECT_TRUE(ValidateSdt(kTestLogTag, sdt));
}
...
TEST(TableValidatorTest, SdtWithZeroTsId) {
  ...
  EXPECT_FALSE(ValidateSdt(kTestLogTag, sdt));
}
...
TEST(TableValidatorTest, BrokenTables) {
  ...
  for (const auto& tc : test_cases) {
    ...
    switch (tc.table_id) {
      case ts::TID_PAT: {
        ts::PAT pat(context, table);
        result = ValidatePat(kTestLogTag, pat, table);
        break;
      }
      case ts::TID_CAT: {
        ts::CAT cat(context, table);
        result = ValidateCat(kTestLogTag, cat);
        break;
      }
      case ts::TID_PMT: {
        ts::PMT pmt(context, table);
        result = ValidatePmt(kTestLogTag, pmt);
        break;
      }
      case ts::TID_EIT_PF_ACT: {
        ts::EIT eit(context, table);
        result = ValidateEit(kTestLogTag, eit);
        break;
      }
      case ts::TID_TOT: {
        ts::TOT tot(context, table);
        result = ValidateTot(kTestLogTag, tot);
        break;
      }
      case ts::TID_NIT_ACT: {
        ts::NIT nit(context, table);
        result = ValidateNit(kTestLogTag, nit);
        break;
      }
      case ts::TID_SDT_ACT: {
        ts::SDT sdt(context, table);
        result = ValidateSdt(kTestLogTag, sdt);
        break;
      }
      default:
